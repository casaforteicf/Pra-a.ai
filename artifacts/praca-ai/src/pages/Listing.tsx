...
const PARTY_SLUG = "festas-e-lembrancinhas"
const GAMES_SLUG = "games"
const MUSIC_SLUG = "instrumentos-musicais"
const JEWELRY_SLUG = "joias-e-relogios"
const BOOKS_SLUG = "livros-revistas-e-comics"
const PET_SLUG = "pet-shop"
...
const partyDepartments = [
  { label: "Aniversário infantil", icon: PartyPopper, search: "festa infantil aniversário" },
  { label: "Balões", icon: CircleDashed, search: "balão bexiga" },
  { label: "Descartáveis", icon: UtensilsCrossed, search: "prato copo descartável" },
  { label: "Decoração temática", icon: Sparkles, search: "decoração tema festa" },
  { label: "Doces e guloseimas", icon: Candy, search: "docinho guloseima festa" },
  { label: "Lembrancinhas", icon: Gift, search: "lembrancinha" },
  { label: "Casamento e noivado", icon: Heart, search: "casamento noivado" },
  { label: "Fantasias", icon: Drama, search: "fantasia carnaval cosplay" },
]

const gamesDepartments = [
  { label: "Consoles", icon: Gamepad2, search: "console videogame" },
  { label: "Jogos físicos", icon: Disc3, search: "jogo físico mídia física" },
  { label: "Cartões e gift cards", icon: Gift, search: "gift card cartão pré-pago jogo" },
  { label: "Controles e acessórios", icon: Joystick, search: "controle acessório videogame" },
  { label: "PC Gamer", icon: Cpu, search: "pc gamer computador gamer" },
  { label: "Cadeiras gamer", icon: Armchair, search: "cadeira gamer" },
  { label: "Jogos de tabuleiro", icon: Dices, search: "jogo de tabuleiro board game" },
  { label: "Retrô", icon: Joystick, search: "console retrô clássico" },
]

const musicDepartments = [
  { label: "Violões e guitarras", icon: Guitar, search: "violão guitarra" },
  { label: "Teclados e pianos", icon: Piano, search: "teclado piano" },
  { label: "Baterias e percussão", icon: Drum, search: "bateria percussão" },
  { label: "Instrumentos de sopro", icon: Wind, search: "instrumento de sopro sax trompete" },
  { label: "Cordas", icon: Music2, search: "violino violoncelo cordas" },
  { label: "Amplificadores", icon: AudioWaveform, search: "amplificador caixa amplificada" },
  { label: "Microfones e áudio", icon: Mic2, search: "microfone interface de áudio" },
  { label: "Acessórios", icon: Cable, search: "acessório instrumento musical corda palheta" },
]

const jewelryDepartments = [
  { label: "Relógios", icon: Watch, search: "relógio" },
  { label: "Anéis", icon: CircleDashed, search: "anel" },
  { label: "Colares e correntes", icon: Link2, search: "colar corrente" },
  { label: "Brincos", icon: Gem, search: "brinco" },
  { label: "Pulseiras", icon: CircleDot, search: "pulseira bracelete" },
  { label: "Alianças", icon: Infinity, search: "aliança casamento noivado" },
  { label: "Pedras preciosas", icon: Diamond, search: "pedra preciosa gema" },
  { label: "Prata e folheados", icon: Sparkle, search: "prata folheado semijoia" },
]

const booksDepartments = [
  { label: "Ficção", icon: BookOpen, search: "romance ficção literatura" },
  { label: "Não ficção", icon: Book, search: "não ficção biografia ensaio" },
  { label: "Infantil e juvenil", icon: BookMarked, search: "livro infantil juvenil" },
  { label: "Didáticos e concursos", icon: GraduationCap, search: "livro didático concurso vestibular" },
  { label: "Quadrinhos e mangás", icon: BookImage, search: "quadrinho mangá HQ" },
  { label: "Revistas", icon: Newspaper, search: "revista" },
  { label: "Religião e espiritualidade", icon: BookHeart, search: "livro religião espiritualidade" },
  { label: "Autoajuda e negócios", icon: BookText, search: "autoajuda negócios livro" },
]

const petDepartments = [
  { label: "Cães", icon: Dog, search: "cachorro cão" },
  { label: "Gatos", icon: Cat, search: "gato felino" },
  { label: "Ração", icon: Bone, search: "ração petisco" },
  { label: "Higiene e banho", icon: Droplets, search: "higiene banho tosa pet" },
  { label: "Brinquedos", icon: Rabbit, search: "brinquedo pet" },
  { label: "Camas e casinhas", icon: Home, search: "cama casinha pet" },
  { label: "Aquarismo", icon: Fish, search: "aquário peixe aquarismo" },
  { label: "Coleiras e acessórios", icon: Cable, search: "coleira guia acessório pet" },
]
...
export default function ListingPage() {
  ...
  const isParty = categorySlug === PARTY_SLUG
  const isGames = categorySlug === GAMES_SLUG
  const isMusic = categorySlug === MUSIC_SLUG
      ...
      <div className="min-h-full w-full bg-background pb-12 text-foreground">
        <header className="bg-primary text-primary-foreground">
          <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between gap-3">
              <button onClick={() => setLocation("/")} className="flex h-10 w-10 items-center justify-center rounded-full bg-primary-foreground/10" aria-label="Voltar"><ChevronLeft className="h-6 w-6" /></button>
              <Link href="/" className="flex items-center gap-2 text-xl font-black"><PartyPopper className="h-7 w-7" /> Praça.ai <span className="hidden text-sm font-semibold text-primary-foreground/70 sm:inline">Festas</span></Link>
              <Link href="/cart" className="relative flex h-10 w-10 items-center justify-center rounded-full bg-primary-foreground/10" aria-label="Carrinho"><ShoppingCart className="h-5 w-5" /></Link>
            </div>
            <form onSubmit={submitSearch} className="relative mt-4">
              <Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground" />
              <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Busque balão, decoração, lembrancinha e mais" className="h-12 border-0 bg-card pl-12 pr-14 text-base text-foreground" />
              <button type="submit" className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md bg-background p-2 text-primary" aria-label="Buscar"><Search className="h-4 w-4" /></button>
            </form>
          </div>
          <div className="border-t border-primary-foreground/15"><div className="mx-auto flex max-w-7xl justify-center gap-5 overflow-x-auto px-4 py-2.5 text-[11px] font-bold sm:gap-10 sm:text-xs"><span className="flex shrink-0 items-center gap-1.5"><ShieldCheck className="h-4 w-4" />Compra segura</span><span className="flex shrink-0 items-center gap-1.5"><PackageCheck className="h-4 w-4" />Lojas curadas da região</span><span className="flex shrink-0 items-center gap-1.5"><Truck className="h-4 w-4" />Entrega local</span></div></div>
        ...
        <main className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
          <section className="overflow-hidden rounded-2xl bg-card p-5 shadow-neon-sm sm:p-8">
            <div className="grid items-center gap-5 md:grid-cols-[1fr_auto]">
              <div><p className="text-xs font-black uppercase tracking-[0.18em] text-primary">Festas e lembrancinhas</p><h1 className="mt-2 max-w-2xl text-3xl font-black leading-tight sm:text-4xl">Toda festa merece um brinde especial</h1><p className="mt-2 max-w-xl text-sm text-muted-foreground sm:text-base">Decoração, descartáveis, doces e lembrancinhas das lojas da sua região.</p><Button onClick={() => document.getElementById("produtos-festas")?.scrollIntoView({ behavior: "smooth" })} className="mt-5 bg-primary font-black text-primary-foreground shadow-neon-sm hover:opacity-90">Ver produtos</Button></div>
              <div className="hidden h-36 w-60 items-center justify-center rounded-2xl border border-border bg-background md:flex"><PartyPopper className="h-24 w-24 text-primary" strokeWidth={1.2} /></div>
            </div>
          </section>

          <section className="mt-5 rounded-2xl bg-card p-4 shadow-sm sm:p-6">
            <div className="flex items-end justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-primary">Encontre mais rápido</p><h2 className="mt-1 text-xl font-black">Por ocasião</h2></div><button onClick={() => { setSearch(""); setSubmittedSearch("") }} className="text-xs font-bold text-primary">Ver tudo</button></div>
            <div className="mt-4 grid grid-cols-4 gap-x-2 gap-y-5 sm:grid-cols-8">{partyDepartments.map(({ label, icon: Icon, search: departmentSearch }) => <button key={label} onClick={() => selectDepartment(departmentSearch)} className="group flex min-w-0 flex-col items-center gap-2 text-center"><span className="flex h-12 w-12 items-center justify-center rounded-full bg-primary/15 text-primary transition group-hover:bg-primary group-hover:text-primary-foreground sm:h-14 sm:w-14"><Icon className="h-6 w-6" /></span><span className="text-[10px] font-bold leading-tight text-foreground sm:text-xs">{label}</span></button>)}</div>
          </section>

          <section id="produtos-festas" className="mt-5 rounded-2xl bg-card p-4 shadow-sm sm:p-6">
            <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-primary">Vitrine de festas</p><h2 className="mt-1 text-2xl font-black">Tudo pra sua celebração</h2>{listData && <p className="mt-1 text-sm text-muted-foreground">{listData.total} {listData.total === 1 ? "produto encontrado" : "produtos encontrados"}{submittedSearch ? ` para “${submittedSearch}”` : ""}</p>}</div></div>
            <div className="-mx-1 mt-4 flex gap-2 overflow-x-auto px-1 pb-2">{sortOptions.slice(0, 5).map((option) => <button key={option.value} onClick={() => setSort(option.value)} className={cn("shrink-0 rounded-full border px-3 py-2 text-xs font-bold", sort === option.value ? "border-primary bg-primary text-primary-foreground shadow-neon-sm" : "bg-card")}>{option.label}</button>)}</div>
            {isLoading ? <div className="py-14"><PageLoader /></div> : isError ? <EmptyCatalog title="Não foi possível carregar os produtos" text="Tente novamente em alguns instantes." clear={() => setSubmittedSearch("")} /> : listData && listData.products.length > 0 ? <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">{listData.products.map((product) => <ProductCard key={product.id} product={product} compact />)}</div> : <div className="mt-5 rounded-2xl border border-dashed border-border bg-background px-5 py-10 text-center"><span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-primary/15 text-primary shadow-sm"><PartyPopper className="h-8 w-8" /></span><h3 className="mt-4 text-lg font-black">{submittedSearch ? "Nenhum produto encontrado" : "Novidades chegando pra sua festa"}</h3><p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">{submittedSearch ? "Tente buscar por outro nome ou escolha uma das categorias acima." : "As lojas parceiras ainda estão publicando o catálogo de festas. Enquanto isso, explore os demais produtos disponíveis na Praça.ai."}</p><div className="mt-5 flex flex-wrap justify-center gap-2"><Button onClick={() => { setSearch(""); setSubmittedSearch("") }} className="bg-primary text-primary-foreground hover:opacity-90">Limpar busca</Button><Button variant="outline" onClick={() => setLocation("/listing")}>Explorar a Praça.ai</Button></div></div>}
          </section>
        </main>
      </div>
    )
      ...
      <div className="min-h-full w-full bg-background pb-12 text-foreground">
        <header className="bg-primary text-primary-foreground">
          <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between gap-3">
              <button onClick={() => setLocation("/")} className="flex h-10 w-10 items-center justify-center rounded-full bg-primary-foreground/10" aria-label="Voltar"><ChevronLeft className="h-6 w-6" /></button>
              <Link href="/" className="flex items-center gap-2 text-xl font-black"><Gamepad2 className="h-7 w-7" /> Praça.ai <span className="hidden text-sm font-semibold text-primary-foreground/70 sm:inline">Games</span></Link>
              <Link href="/cart" className="relative flex h-10 w-10 items-center justify-center rounded-full bg-primary-foreground/10" aria-label="Carrinho"><ShoppingCart className="h-5 w-5" /></Link>
            </div>
            <form onSubmit={submitSearch} className="relative mt-4">
              <Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground" />
              <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Busque console, jogo, controle e mais" className="h-12 border-0 bg-card pl-12 pr-14 text-base text-foreground" />
              <button type="submit" className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md bg-background p-2 text-primary" aria-label="Buscar"><Search className="h-4 w-4" /></button>
            </form>
          </div>
          <div className="border-t border-primary-foreground/15"><div className="mx-auto flex max-w-7xl justify-center gap-5 overflow-x-auto px-4 py-2.5 text-[11px] font-bold sm:gap-10 sm:text-xs"><span className="flex shrink-0 items-center gap-1.5"><ShieldCheck className="h-4 w-4" />Compra segura</span><span className="flex shrink-0 items-center gap-1.5"><PackageCheck className="h-4 w-4" />Lojas curadas da região</span><span className="flex shrink-0 items-center gap-1.5"><Truck className="h-4 w-4" />Entrega local</span></div></div>
        ...
        <main className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
          <section className="overflow-hidden rounded-2xl bg-card p-5 shadow-neon-sm sm:p-8">
            <div className="grid items-center gap-5 md:grid-cols-[1fr_auto]">
              <div><p className="text-xs font-black uppercase tracking-[0.18em] text-primary">Games</p><h1 className="mt-2 max-w-2xl text-3xl font-black leading-tight sm:text-4xl">Level up no seu setup</h1><p className="mt-2 max-w-xl text-sm text-muted-foreground sm:text-base">Consoles, jogos, acessórios e PC gamer das lojas da sua região.</p><Button onClick={() => document.getElementById("produtos-games")?.scrollIntoView({ behavior: "smooth" })} className="mt-5 bg-primary font-black text-primary-foreground shadow-neon-sm hover:opacity-90">Ver produtos</Button></div>
              <div className="hidden h-36 w-60 items-center justify-center rounded-2xl border border-border bg-background md:flex"><Gamepad2 className="h-24 w-24 text-primary" strokeWidth={1.2} /></div>
            </div>
          </section>

          <section className="mt-5 rounded-2xl bg-card p-4 shadow-sm sm:p-6">
            <div className="flex items-end justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-primary">Encontre mais rápido</p><h2 className="mt-1 text-xl font-black">Por categoria</h2></div><button onClick={() => { setSearch(""); setSubmittedSearch("") }} className="text-xs font-bold text-primary">Ver tudo</button></div>
            <div className="mt-4 grid grid-cols-4 gap-x-2 gap-y-5 sm:grid-cols-8">{gamesDepartments.map(({ label, icon: Icon, search: departmentSearch }) => <button key={label} onClick={() => selectDepartment(departmentSearch)} className="group flex min-w-0 flex-col items-center gap-2 text-center"><span className="flex h-12 w-12 items-center justify-center rounded-full bg-primary/15 text-primary transition group-hover:bg-primary group-hover:text-primary-foreground sm:h-14 sm:w-14"><Icon className="h-6 w-6" /></span><span className="text-[10px] font-bold leading-tight text-foreground sm:text-xs">{label}</span></button>)}</div>
          </section>

          <section id="produtos-games" className="mt-5 rounded-2xl bg-card p-4 shadow-sm sm:p-6">
            <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-primary">Vitrine gamer</p><h2 className="mt-1 text-2xl font-black">Consoles e jogos</h2>{listData && <p className="mt-1 text-sm text-muted-foreground">{listData.total} {listData.total === 1 ? "produto encontrado" : "produtos encontrados"}{submittedSearch ? ` para “${submittedSearch}”` : ""}</p>}</div></div>
            <div className="-mx-1 mt-4 flex gap-2 overflow-x-auto px-1 pb-2">{sortOptions.slice(0, 5).map((option) => <button key={option.value} onClick={() => setSort(option.value)} className={cn("shrink-0 rounded-full border px-3 py-2 text-xs font-bold", sort === option.value ? "border-primary bg-primary text-primary-foreground shadow-neon-sm" : "bg-card")}>{option.label}</button>)}</div>
            {isLoading ? <div className="py-14"><PageLoader /></div> : isError ? <EmptyCatalog title="Não foi possível carregar os produtos" text="Tente novamente em alguns instantes." clear={() => setSubmittedSearch("")} /> : listData && listData.products.length > 0 ? <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">{listData.products.map((product) => <ProductCard key={product.id} product={product} compact />)}</div> : <div className="mt-5 rounded-2xl border border-dashed border-border bg-background px-5 py-10 text-center"><span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-primary/15 text-primary shadow-sm"><Gamepad2 className="h-8 w-8" /></span><h3 className="mt-4 text-lg font-black">{submittedSearch ? "Nenhum produto encontrado" : "Novos games chegando"}</h3><p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">{submittedSearch ? "Tente buscar por outro nome ou escolha uma das categorias acima." : "As lojas parceiras ainda estão publicando o catálogo gamer. Enquanto isso, explore os demais produtos disponíveis na Praça.ai."}</p><div className="mt-5 flex flex-wrap justify-center gap-2"><Button onClick={() => { setSearch(""); setSubmittedSearch("") }} className="bg-primary text-primary-foreground hover:opacity-90">Limpar busca</Button><Button variant="outline" onClick={() => setLocation("/listing")}>Explorar a Praça.ai</Button></div></div>}
          </section>
        </main>
      </div>
    )
      ...
      <div className="min-h-full w-full bg-background pb-12 text-foreground">
        <header className="bg-primary text-primary-foreground">
          <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between gap-3">
              <button onClick={() => setLocation("/")} className="flex h-10 w-10 items-center justify-center rounded-full bg-primary-foreground/10" aria-label="Voltar"><ChevronLeft className="h-6 w-6" /></button>
              <Link href="/" className="flex items-center gap-2 text-xl font-black"><Guitar className="h-7 w-7" /> Praça.ai <span className="hidden text-sm font-semibold text-primary-foreground/70 sm:inline">Música</span></Link>
              <Link href="/cart" className="relative flex h-10 w-10 items-center justify-center rounded-full bg-primary-foreground/10" aria-label="Carrinho"><ShoppingCart className="h-5 w-5" /></Link>
            </div>
            <form onSubmit={submitSearch} className="relative mt-4">
              <Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground" />
              <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Busque violão, teclado, bateria e mais" className="h-12 border-0 bg-card pl-12 pr-14 text-base text-foreground" />
              <button type="submit" className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md bg-background p-2 text-primary" aria-label="Buscar"><Search className="h-4 w-4" /></button>
            </form>
          </div>
          <div className="border-t border-primary-foreground/15"><div className="mx-auto flex max-w-7xl justify-center gap-5 overflow-x-auto px-4 py-2.5 text-[11px] font-bold sm:gap-10 sm:text-xs"><span className="flex shrink-0 items-center gap-1.5"><ShieldCheck className="h-4 w-4" />Compra segura</span><span className="flex shrink-0 items-center gap-1.5"><PackageCheck className="h-4 w-4" />Lojas curadas da região</span><span className="flex shrink-0 items-center gap-1.5"><Truck className="h-4 w-4" />Entrega local</span></div></div>
        ...
        <main className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
          <section className="overflow-hidden rounded-2xl bg-card p-5 shadow-neon-sm sm:p-8">
            <div className="grid items-center gap-5 md:grid-cols-[1fr_auto]">
              <div><p className="text-xs font-black uppercase tracking-[0.18em] text-primary">Instrumentos musicais</p><h1 className="mt-2 max-w-2xl text-3xl font-black leading-tight sm:text-4xl">Encontre seu próximo instrumento</h1><p className="mt-2 max-w-xl text-sm text-muted-foreground sm:text-base">Cordas, teclas, sopro, percussão e áudio das lojas da sua região.</p><Button onClick={() => document.getElementById("produtos-musica")?.scrollIntoView({ behavior: "smooth" })} className="mt-5 bg-primary font-black text-primary-foreground shadow-neon-sm hover:opacity-90">Ver produtos</Button></div>
              <div className="hidden h-36 w-60 items-center justify-center rounded-2xl border border-border bg-background md:flex"><Guitar className="h-24 w-24 text-primary" strokeWidth={1.2} /></div>
            </div>
          </section>

          <section className="mt-5 rounded-2xl bg-card p-4 shadow-sm sm:p-6">
            <div className="flex items-end justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-primary">Encontre mais rápido</p><h2 className="mt-1 text-xl font-black">Por categoria</h2></div><button onClick={() => { setSearch(""); setSubmittedSearch("") }} className="text-xs font-bold text-primary">Ver tudo</button></div>
            <div className="mt-4 grid grid-cols-4 gap-x-2 gap-y-5 sm:grid-cols-8">{musicDepartments.map(({ label, icon: Icon, search: departmentSearch }) => <button key={label} onClick={() => selectDepartment(departmentSearch)} className="group flex min-w-0 flex-col items-center gap-2 text-center"><span className="flex h-12 w-12 items-center justify-center rounded-full bg-primary/15 text-primary transition group-hover:bg-primary group-hover:text-primary-foreground sm:h-14 sm:w-14"><Icon className="h-6 w-6" /></span><span className="text-[10px] font-bold leading-tight text-foreground sm:text-xs">{label}</span></button>)}</div>
          </section>

          <section id="produtos-musica" className="mt-5 rounded-2xl bg-card p-4 shadow-sm sm:p-6">
            <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-primary">Vitrine musical</p><h2 className="mt-1 text-2xl font-black">Instrumentos e acessórios</h2>{listData && <p className="mt-1 text-sm text-muted-foreground">{listData.total} {listData.total === 1 ? "produto encontrado" : "produtos encontrados"}{submittedSearch ? ` para “${submittedSearch}”` : ""}</p>}</div></div>
            <div className="-mx-1 mt-4 flex gap-2 overflow-x-auto px-1 pb-2">{sortOptions.slice(0, 5).map((option) => <button key={option.value} onClick={() => setSort(option.value)} className={cn("shrink-0 rounded-full border px-3 py-2 text-xs font-bold", sort === option.value ? "border-primary bg-primary text-primary-foreground shadow-neon-sm" : "bg-card")}>{option.label}</button>)}</div>
            {isLoading ? <div className="py-14"><PageLoader /></div> : isError ? <EmptyCatalog title="Não foi possível carregar os produtos" text="Tente novamente em alguns instantes." clear={() => setSubmittedSearch("")} /> : listData && listData.products.length > 0 ? <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">{listData.products.map((product) => <ProductCard key={product.id} product={product} compact />)}</div> : <div className="mt-5 rounded-2xl border border-dashed border-border bg-background px-5 py-10 text-center"><span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-primary/15 text-primary shadow-sm"><Guitar className="h-8 w-8" /></span><h3 className="mt-4 text-lg font-black">{submittedSearch ? "Nenhum produto encontrado" : "Novos instrumentos chegando"}</h3><p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">{submittedSearch ? "Tente buscar por outro nome ou escolha uma das categorias acima." : "As lojas parceiras ainda estão publicando o catálogo musical. Enquanto isso, explore os demais produtos disponíveis na Praça.ai."}</p><div className="mt-5 flex flex-wrap justify-center gap-2"><Button onClick={() => { setSearch(""); setSubmittedSearch("") }} className="bg-primary text-primary-foreground hover:opacity-90">Limpar busca</Button><Button variant="outline" onClick={() => setLocation("/listing")}>Explorar a Praça.ai</Button></div></div>}
          </section>
        </main>
      </div>
    )
      ...
      <div className="min-h-full w-full bg-background pb-12 text-foreground">
        <header className="bg-primary text-primary-foreground">
          <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between gap-3">
              <button onClick={() => setLocation("/")} className="flex h-10 w-10 items-center justify-center rounded-full bg-primary-foreground/10" aria-label="Voltar"><ChevronLeft className="h-6 w-6" /></button>
              <Link href="/" className="flex items-center gap-2 text-xl font-black"><Gem className="h-7 w-7" /> Praça.ai <span className="hidden text-sm font-semibold text-primary-foreground/70 sm:inline">Joias</span></Link>
              <Link href="/cart" className="relative flex h-10 w-10 items-center justify-center rounded-full bg-primary-foreground/10" aria-label="Carrinho"><ShoppingCart className="h-5 w-5" /></Link>
            </div>
            <form onSubmit={submitSearch} className="relative mt-4">
              <Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground" />
              <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Busque relógio, anel, aliança e mais" className="h-12 border-0 bg-card pl-12 pr-14 text-base text-foreground" />
              <button type="submit" className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md bg-background p-2 text-primary" aria-label="Buscar"><Search className="h-4 w-4" /></button>
            </form>
          </div>
          <div className="border-t border-primary-foreground/15"><div className="mx-auto flex max-w-7xl justify-center gap-5 overflow-x-auto px-4 py-2.5 text-[11px] font-bold sm:gap-10 sm:text-xs"><span className="flex shrink-0 items-center gap-1.5"><ShieldCheck className="h-4 w-4" />Compra segura</span><span className="flex shrink-0 items-center gap-1.5"><PackageCheck className="h-4 w-4" />Lojas curadas da região</span><span className="flex shrink-0 items-center gap-1.5"><Truck className="h-4 w-4" />Entrega local</span></div></div>
        ...
        <main className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
          <section className="overflow-hidden rounded-2xl bg-card p-5 shadow-neon-sm sm:p-8">
            <div className="grid items-center gap-5 md:grid-cols-[1fr_auto]">
              <div><p className="text-xs font-black uppercase tracking-[0.18em] text-primary">Joias e relógios</p><h1 className="mt-2 max-w-2xl text-3xl font-black leading-tight sm:text-4xl">Um brilho a mais pra cada momento</h1><p className="mt-2 max-w-xl text-sm text-muted-foreground sm:text-base">Relógios, anéis, alianças e semijoias das lojas da sua região.</p><Button onClick={() => document.getElementById("produtos-joias")?.scrollIntoView({ behavior: "smooth" })} className="mt-5 bg-primary font-black text-primary-foreground shadow-neon-sm hover:opacity-90">Ver produtos</Button></div>
              <div className="hidden h-36 w-60 items-center justify-center rounded-2xl border border-border bg-background md:flex"><Gem className="h-24 w-24 text-primary" strokeWidth={1.2} /></div>
            </div>
          </section>

          <section className="mt-5 rounded-2xl bg-card p-4 shadow-sm sm:p-6">
            <div className="flex items-end justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-primary">Encontre mais rápido</p><h2 className="mt-1 text-xl font-black">Por categoria</h2></div><button onClick={() => { setSearch(""); setSubmittedSearch("") }} className="text-xs font-bold text-primary">Ver tudo</button></div>
            <div className="mt-4 grid grid-cols-4 gap-x-2 gap-y-5 sm:grid-cols-8">{jewelryDepartments.map(({ label, icon: Icon, search: departmentSearch }) => <button key={label} onClick={() => selectDepartment(departmentSearch)} className="group flex min-w-0 flex-col items-center gap-2 text-center"><span className="flex h-12 w-12 items-center justify-center rounded-full bg-primary/15 text-primary transition group-hover:bg-primary group-hover:text-primary-foreground sm:h-14 sm:w-14"><Icon className="h-6 w-6" /></span><span className="text-[10px] font-bold leading-tight text-foreground sm:text-xs">{label}</span></button>)}</div>
          </section>

          <section id="produtos-joias" className="mt-5 rounded-2xl bg-card p-4 shadow-sm sm:p-6">
            <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-primary">Vitrine de joias</p><h2 className="mt-1 text-2xl font-black">Joias e relógios</h2>{listData && <p className="mt-1 text-sm text-muted-foreground">{listData.total} {listData.total === 1 ? "produto encontrado" : "produtos encontrados"}{submittedSearch ? ` para “${submittedSearch}”` : ""}</p>}</div></div>
            <div className="-mx-1 mt-4 flex gap-2 overflow-x-auto px-1 pb-2">{sortOptions.slice(0, 5).map((option) => <button key={option.value} onClick={() => setSort(option.value)} className={cn("shrink-0 rounded-full border px-3 py-2 text-xs font-bold", sort === option.value ? "border-primary bg-primary text-primary-foreground shadow-neon-sm" : "bg-card")}>{option.label}</button>)}</div>
            {isLoading ? <div className="py-14"><PageLoader /></div> : isError ? <EmptyCatalog title="Não foi possível carregar os produtos" text="Tente novamente em alguns instantes." clear={() => setSubmittedSearch("")} /> : listData && listData.products.length > 0 ? <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">{listData.products.map((product) => <ProductCard key={product.id} product={product} compact />)}</div> : <div className="mt-5 rounded-2xl border border-dashed border-border bg-background px-5 py-10 text-center"><span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-primary/15 text-primary shadow-sm"><Gem className="h-8 w-8" /></span><h3 className="mt-4 text-lg font-black">{submittedSearch ? "Nenhum produto encontrado" : "Novas peças chegando"}</h3><p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">{submittedSearch ? "Tente buscar por outro nome ou escolha uma das categorias acima." : "As lojas parceiras ainda estão publicando o catálogo de joias. Enquanto isso, explore os demais produtos disponíveis na Praça.ai."}</p><div className="mt-5 flex flex-wrap justify-center gap-2"><Button onClick={() => { setSearch(""); setSubmittedSearch("") }} className="bg-primary text-primary-foreground hover:opacity-90">Limpar busca</Button><Button variant="outline" onClick={() => setLocation("/listing")}>Explorar a Praça.ai</Button></div></div>}
          </section>
        </main>
      </div>
    )
      ...
      <div className="min-h-full w-full bg-background pb-12 text-foreground">
        <header className="bg-primary text-primary-foreground">
          <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between gap-3">
              <button onClick={() => setLocation("/")} className="flex h-10 w-10 items-center justify-center rounded-full bg-primary-foreground/10" aria-label="Voltar"><ChevronLeft className="h-6 w-6" /></button>
              <Link href="/" className="flex items-center gap-2 text-xl font-black"><Book className="h-7 w-7" /> Praça.ai <span className="hidden text-sm font-semibold text-primary-foreground/70 sm:inline">Livros</span></Link>
              <Link href="/cart" className="relative flex h-10 w-10 items-center justify-center rounded-full bg-primary-foreground/10" aria-label="Carrinho"><ShoppingCart className="h-5 w-5" /></Link>
            </div>
            <form onSubmit={submitSearch} className="relative mt-4">
              <Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground" />
              <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Busque livro, autor, revista e mais" className="h-12 border-0 bg-card pl-12 pr-14 text-base text-foreground" />
              <button type="submit" className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md bg-background p-2 text-primary" aria-label="Buscar"><Search className="h-4 w-4" /></button>
            </form>
          </div>
          <div className="border-t border-primary-foreground/15"><div className="mx-auto flex max-w-7xl justify-center gap-5 overflow-x-auto px-4 py-2.5 text-[11px] font-bold sm:gap-10 sm:text-xs"><span className="flex shrink-0 items-center gap-1.5"><ShieldCheck className="h-4 w-4" />Compra segura</span><span className="flex shrink-0 items-center gap-1.5"><PackageCheck className="h-4 w-4" />Lojas curadas da região</span><span className="flex shrink-0 items-center gap-1.5"><Truck className="h-4 w-4" />Entrega local</span></div></div>
        ...
        <main className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
          <section className="overflow-hidden rounded-2xl bg-card p-5 shadow-neon-sm sm:p-8">
            <div className="grid items-center gap-5 md:grid-cols-[1fr_auto]">
              <div><p className="text-xs font-black uppercase tracking-[0.18em] text-primary">Livros, revistas e comics</p><h1 className="mt-2 max-w-2xl text-3xl font-black leading-tight sm:text-4xl">Sua próxima leitura está aqui</h1><p className="mt-2 max-w-xl text-sm text-muted-foreground sm:text-base">Romances, didáticos, quadrinhos e revistas das lojas e sebos da sua região.</p><Button onClick={() => document.getElementById("produtos-livros")?.scrollIntoView({ behavior: "smooth" })} className="mt-5 bg-primary font-black text-primary-foreground shadow-neon-sm hover:opacity-90">Ver produtos</Button></div>
              <div className="hidden h-36 w-60 items-center justify-center rounded-2xl border border-border bg-background md:flex"><Book className="h-24 w-24 text-primary" strokeWidth={1.2} /></div>
            </div>
          </section>

          <section className="mt-5 rounded-2xl bg-card p-4 shadow-sm sm:p-6">
            <div className="flex items-end justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-primary">Encontre mais rápido</p><h2 className="mt-1 text-xl font-black">Por categoria</h2></div><button onClick={() => { setSearch(""); setSubmittedSearch("") }} className="text-xs font-bold text-primary">Ver tudo</button></div>
            <div className="mt-4 grid grid-cols-4 gap-x-2 gap-y-5 sm:grid-cols-8">{booksDepartments.map(({ label, icon: Icon, search: departmentSearch }) => <button key={label} onClick={() => selectDepartment(departmentSearch)} className="group flex min-w-0 flex-col items-center gap-2 text-center"><span className="flex h-12 w-12 items-center justify-center rounded-full bg-primary/15 text-primary transition group-hover:bg-primary group-hover:text-primary-foreground sm:h-14 sm:w-14"><Icon className="h-6 w-6" /></span><span className="text-[10px] font-bold leading-tight text-foreground sm:text-xs">{label}</span></button>)}</div>
          </section>

          <section id="produtos-livros" className="mt-5 rounded-2xl bg-card p-4 shadow-sm sm:p-6">
            <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-primary">Vitrine literária</p><h2 className="mt-1 text-2xl font-black">Livros, revistas e comics</h2>{listData && <p className="mt-1 text-sm text-muted-foreground">{listData.total} {listData.total === 1 ? "produto encontrado" : "produtos encontrados"}{submittedSearch ? ` para “${submittedSearch}”` : ""}</p>}</div></div>
            <div className="-mx-1 mt-4 flex gap-2 overflow-x-auto px-1 pb-2">{sortOptions.slice(0, 5).map((option) => <button key={option.value} onClick={() => setSort(option.value)} className={cn("shrink-0 rounded-full border px-3 py-2 text-xs font-bold", sort === option.value ? "border-primary bg-primary text-primary-foreground shadow-neon-sm" : "bg-card")}>{option.label}</button>)}</div>
            {isLoading ? <div className="py-14"><PageLoader /></div> : isError ? <EmptyCatalog title="Não foi possível carregar os produtos" text="Tente novamente em alguns instantes." clear={() => setSubmittedSearch("")} /> : listData && listData.products.length > 0 ? <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">{listData.products.map((product) => <ProductCard key={product.id} product={product} compact />)}</div> : <div className="mt-5 rounded-2xl border border-dashed border-border bg-background px-5 py-10 text-center"><span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-primary/15 text-primary shadow-sm"><Book className="h-8 w-8" /></span><h3 className="mt-4 text-lg font-black">{submittedSearch ? "Nenhum produto encontrado" : "Novos títulos chegando"}</h3><p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">{submittedSearch ? "Tente buscar por outro nome ou escolha uma das categorias acima." : "As lojas parceiras ainda estão publicando o catálogo literário. Enquanto isso, explore os demais produtos disponíveis na Praça.ai."}</p><div className="mt-5 flex flex-wrap justify-center gap-2"><Button onClick={() => { setSearch(""); setSubmittedSearch("") }} className="bg-primary text-primary-foreground hover:opacity-90">Limpar busca</Button><Button variant="outline" onClick={() => setLocation("/listing")}>Explorar a Praça.ai</Button></div></div>}
          </section>
        </main>
      </div>
    )
      ...
      <div className="min-h-full w-full bg-background pb-12 text-foreground">
        <header className="bg-primary text-primary-foreground">
          <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between gap-3">
              <button onClick={() => setLocation("/")} className="flex h-10 w-10 items-center justify-center rounded-full bg-primary-foreground/10" aria-label="Voltar"><ChevronLeft className="h-6 w-6" /></button>
              <Link href="/" className="flex items-center gap-2 text-xl font-black"><Dog className="h-7 w-7" /> Praça.ai <span className="hidden text-sm font-semibold text-primary-foreground/70 sm:inline">Pet Shop</span></Link>
              <Link href="/cart" className="relative flex h-10 w-10 items-center justify-center rounded-full bg-primary-foreground/10" aria-label="Carrinho"><ShoppingCart className="h-5 w-5" /></Link>
            </div>
            <form onSubmit={submitSearch} className="relative mt-4">
              <Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground" />
              <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Busque ração, brinquedo, banho e mais" className="h-12 border-0 bg-card pl-12 pr-14 text-base text-foreground" />
              <button type="submit" className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md bg-background p-2 text-primary" aria-label="Buscar"><Search className="h-4 w-4" /></button>
            </form>
          </div>
          <div className="border-t border-primary-foreground/15"><div className="mx-auto flex max-w-7xl justify-center gap-5 overflow-x-auto px-4 py-2.5 text-[11px] font-bold sm:gap-10 sm:text-xs"><span className="flex shrink-0 items-center gap-1.5"><ShieldCheck className="h-4 w-4" />Compra segura</span><span className="flex shrink-0 items-center gap-1.5"><PackageCheck className="h-4 w-4" />Lojas curadas da região</span><span className="flex shrink-0 items-center gap-1.5"><Truck className="h-4 w-4" />Entrega local</span></div></div>
        ...
        <main className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
          <section className="overflow-hidden rounded-2xl bg-card p-5 shadow-neon-sm sm:p-8">
            <div className="grid items-center gap-5 md:grid-cols-[1fr_auto]">
              <div><p className="text-xs font-black uppercase tracking-[0.18em] text-primary">Pet Shop</p><h1 className="mt-2 max-w-2xl text-3xl font-black leading-tight sm:text-4xl">Tudo pro seu melhor amigo</h1><p className="mt-2 max-w-xl text-sm text-muted-foreground sm:text-base">Ração, brinquedos, higiene e acessórios das lojas da sua região.</p><Button onClick={() => document.getElementById("produtos-pet")?.scrollIntoView({ behavior: "smooth" })} className="mt-5 bg-primary font-black text-primary-foreground shadow-neon-sm hover:opacity-90">Ver produtos</Button></div>
              <div className="hidden h-36 w-60 items-center justify-center rounded-2xl border border-border bg-background md:flex"><Dog className="h-24 w-24 text-primary" strokeWidth={1.2} /></div>
            </div>
          </section>

          <section className="mt-5 rounded-2xl bg-card p-4 shadow-sm sm:p-6">
            <div className="flex items-end justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-primary">Encontre mais rápido</p><h2 className="mt-1 text-xl font-black">Por categoria</h2></div><button onClick={() => { setSearch(""); setSubmittedSearch("") }} className="text-xs font-bold text-primary">Ver tudo</button></div>
            <div className="mt-4 grid grid-cols-4 gap-x-2 gap-y-5 sm:grid-cols-8">{petDepartments.map(({ label, icon: Icon, search: departmentSearch }) => <button key={label} onClick={() => selectDepartment(departmentSearch)} className="group flex min-w-0 flex-col items-center gap-2 text-center"><span className="flex h-12 w-12 items-center justify-center rounded-full bg-primary/15 text-primary transition group-hover:bg-primary group-hover:text-primary-foreground sm:h-14 sm:w-14"><Icon className="h-6 w-6" /></span><span className="text-[10px] font-bold leading-tight text-foreground sm:text-xs">{label}</span></button>)}</div>
          </section>

          <section className="mt-5 rounded-2xl bg-card p-6 shadow-sm">
            <div className="flex items-center gap-4">
              <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-primary/15 text-primary"><Droplets className="h-7 w-7" /></span>
              <div className="flex-1"><p className="text-xs font-black uppercase tracking-[0.16em] text-primary">Banho e tosa</p><h3 className="mt-0.5 text-lg font-black">Agende direto com o pet shop</h3><p className="mt-1 text-sm text-muted-foreground">Lojas parceiras com serviço de banho e tosa aparecem na busca — procure pela categoria "Higiene e banho".</p></div>
            </div>
          </section>

          <section id="produtos-pet" className="mt-5 rounded-2xl bg-card p-4 shadow-sm sm:p-6">
            <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-primary">Vitrine pet</p><h2 className="mt-1 text-2xl font-black">Ração, higiene e diversão</h2>{listData && <p className="mt-1 text-sm text-muted-foreground">{listData.total} {listData.total === 1 ? "produto encontrado" : "produtos encontrados"}{submittedSearch ? ` para “${submittedSearch}”` : ""}</p>}</div></div>
            <div className="-mx-1 mt-4 flex gap-2 overflow-x-auto px-1 pb-2">{sortOptions.slice(0, 5).map((option) => <button key={option.value} onClick={() => setSort(option.value)} className={cn("shrink-0 rounded-full border px-3 py-2 text-xs font-bold", sort === option.value ? "border-primary bg-primary text-primary-foreground shadow-neon-sm" : "bg-card")}>{option.label}</button>)}</div>
            {isLoading ? <div className="py-14"><PageLoader /></div> : isError ? <EmptyCatalog title="Não foi possível carregar os produtos" text="Tente novamente em alguns instantes." clear={() => setSubmittedSearch("")} /> : listData && listData.products.length > 0 ? <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">{listData.products.map((product) => <ProductCard key={product.id} product={product} compact />)}</div> : <div className="mt-5 rounded-2xl border border-dashed border-border bg-background px-5 py-10 text-center"><span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-primary/15 text-primary shadow-sm"><Dog className="h-8 w-8" /></span><h3 className="mt-4 text-lg font-black">{submittedSearch ? "Nenhum produto encontrado" : "Novos produtos chegando"}</h3><p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">{submittedSearch ? "Tente buscar por outro nome ou escolha uma das categorias acima." : "As lojas parceiras ainda estão publicando o catálogo pet. Enquanto isso, explore os demais produtos disponíveis na Praça.ai."}</p><div className="mt-5 flex flex-wrap justify-center gap-2"><Button onClick={() => { setSearch(""); setSubmittedSearch("") }} className="bg-primary text-primary-foreground hover:opacity-90">Limpar busca</Button><Button variant="outline" onClick={() => setLocation("/listing")}>Explorar a Praça.ai</Button></div></div>}
          </section>
        </main>
      </div>
    )
